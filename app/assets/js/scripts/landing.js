...
document.getElementById('launch_button').addEventListener('click', function(e){
    ...
    if(jExe == null){
        ...
        jg._validateJavaBinary(jExe).then((v) => {
            loggerLanding.log('Version méta de Java', v)
            if(v.valid){
                dlAsync()
            } else {
                asyncSystemScan(mcVersion)
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    'Aucune installation compatible <br> de Java trouvée',
                    'Pour rejoindre Nezolia, vous avez besoin d\'une installation 64 bits de Java 8. Souhaitez-vous que nous en installions une copie? En installant, vous acceptez le <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">Contrat de licence d\'Oracle</a>.',
                    'Installer Java',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            'Java est requis <br> pour se lancer',
                            'Une installation x64 valide de Java 8 est requise pour démarrer. <br> <br> Veuillez consulter notre <a href="https://github.com/dscalzi/HeliosLauncher/wiki/Java-Management#manually-installing-a-valid-version-of-java">Guide de gestion Java</a> pour savoir comment installer manuellement Java.',
                            'Je comprends',
                            'Retour'
                        )
                        setOverlayHandler(() => {
        ...
        if(m.context === 'validate'){
            switch(m.data){
                ...
                case 'version':
                    ...
                    setLaunchDetails('Valider l\'intégrité des asset...')
                    break
                case 'assets':
                    setLaunchPercentage(60, 100)
                    loggerLaunchSuite.log('Validation des asset terminée')
                    setLaunchDetails('Validation de l\'intégrité des bibliothèques...')
                    break
                case 'libraries':
                    setLaunchPercentage(80, 100)
                    loggerLaunchSuite.log('Validation des bibliothèques terminée.')
                    setLaunchDetails('Validation de l\'intégrité des fichiers divers...')
                    break
                case 'files':
                    setLaunchPercentage(100, 100)
                    loggerLaunchSuite.log('Validation des fichiers terminée.')
            ...
            switch(m.data){
                case 'download':
                    ...
                    setLaunchDetails('Se préparer au lancement...')
                    break
            }
        } else if(m.context === 'error'){
            switch(m.data){
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        ...
                        DiscordWrapper.updateDetails('Navigation vers Nezolia!')
                    }
                }
